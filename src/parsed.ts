...
export type ParsedFieldCondition =
  ...
  /**
   * Membership: wire form `field: [v1, v2, ...]` or object form
   * `field: { in: [...] }` → SQL IN; `field: { notIn: [...] }` → SQL NOT IN
   * (`negate: true`). string/id/enum/number only.
   */
  | { kind: "in"; field: string; values: (string | number)[]; negate?: boolean }
